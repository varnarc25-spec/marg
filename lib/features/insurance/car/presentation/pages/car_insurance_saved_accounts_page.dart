...
/// Car Insurance saved accounts (view + select).
class CarInsuranceSavedAccountsPage extends ConsumerWidget {
  const CarInsuranceSavedAccountsPage({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: async.when(
        ...
        data: (accounts) {
          // Silent when there are no accounts.
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (selected) ...[
                        const SizedBox(height: 12),
                        if (a.masked != null && a.masked!.isNotEmpty)
                          Text(
                            'Account: ${a.masked}',
                            style: textTheme.bodySmall?.copyWith(
                              color: colorScheme.onSurfaceVariant,
  ...
  Future<void> _showCreateAccountDialog(BuildContext context, WidgetRef ref) async {
    final mut = ref.read(carSavedAccountsMutationsProvider);
    final theme = Theme.of(context);
                ...
                FilledButton(
                  onPressed: () async {
                    ...
                    if (accountType.isEmpty || title.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Account Type and Title are required.'),
                        ),
                      );
                      return;
                    }
