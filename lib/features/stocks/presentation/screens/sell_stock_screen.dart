...
/// Purple accent for sell button (per design)
const Color _sellPurple = Color(0xFF6C63FF);

/// Sell Stock – Screen for entering sell amount and placing a sell order (e.g. AMZN)
...
class _SellStockScreenState extends State<SellStockScreen> {
  ...
  static const double _assetBalance = 4987.00; // User's AMZN holding value
  static const String _symbol = 'AMZN';

  // Yellow circle for Amazon logo (per design)
  static const Color _amazonYellow = Color(0xFFFFC107);

  String _inputString = '675.00';
  bool _isSharesMode = false; // true = input is shares, false = input is USD amount
  ...
  void _onPercentageTap(double pct) {
    setState(() {
      final amount = _assetBalance * pct;
      if (_isSharesMode) {
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: _amazonYellow.withValues(alpha: 0.4),
                blurRadius: 8,
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                'Asset balance ${_formatCurrency(_assetBalance)}',
  ...
  Widget _buildSellButton(BuildContext context) {
    ...
    final canSell = _shares > 0 && total <= _assetBalance;
    return Padding(
