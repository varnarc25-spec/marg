/// Fetched bill from `POST /api/utilities/electricity/fetch-bill`.
class ElectricityBill {
  ...
  /// Display name (same as [consumerName]).
  ...
  factory ElectricityBill.fromApiJson(Map<String, dynamic> json) {
    final consumer = (json['consumerNumber'] ??
            json['consumerId'] ??
            json['accountNumber'] ??
            '')
        .toString();
    final nm = (json['consumerName'] ?? json['name'] ?? json['customerName'] ?? 'Consumer')
        .toString();
    final amt = _toDouble(json['amount'] ?? json['billAmount'] ?? json['payableAmount']);
    ...
    final breakdown = (json['breakdown'] ?? json['summary'] ?? json['description'] ?? '')
        .toString();
    ...
    return ElectricityBill(
      consumerNumber: consumer.isNotEmpty ? consumer : '—',
      consumerName: nm,
      amount: amt,
      dueDate: due,
      billPeriod: period,
