...
/// `POST /api/utilities/electricity/accounts`
class ElectricityAddAccountPage extends ConsumerStatefulWidget {
  const ElectricityAddAccountPage({super.key});

  @override
  ConsumerState<ElectricityAddAccountPage> createState() =>
      _ElectricityAddAccountPageState();
}

class _ElectricityAddAccountPageState
    extends ConsumerState<ElectricityAddAccountPage> {
  ...
  Future<void> _save() async {
    ...
    if (biller == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Select a biller from Electricity flow first')),
      );
      return;
    }
    ...
    try {
      await ref.read(electricityRepositoryProvider).createAccount(
            accountNumber: acc,
            ...
            consumerName: _consumerNameController.text.trim().isEmpty
                ? '—'
                : _consumerNameController.text.trim(),
            isAutopay: _autopay,
          );
      ref.invalidate(electricitySavedAccountsProvider);
      if (!mounted) return;
      Navigator.of(context).pop(true);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(electricityApiUserMessage(e))),
      );
    } finally {
  ...
  Widget build(BuildContext context) {
    final biller = ref.watch(selectedElectricityBillerProvider);
    return Scaffold(
