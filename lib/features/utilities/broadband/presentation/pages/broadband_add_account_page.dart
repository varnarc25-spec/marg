...
/// `POST /api/utilities/broadband-landline/accounts`
class BroadbandAddAccountPage extends ConsumerStatefulWidget {
  const BroadbandAddAccountPage({super.key});

  @override
  ConsumerState<BroadbandAddAccountPage> createState() => _BroadbandAddAccountPageState();
}

class _BroadbandAddAccountPageState extends ConsumerState<BroadbandAddAccountPage> {
  ...
  Future<void> _save() async {
    ...
    if (biller == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Select a biller from Broadband flow first')),
      );
      return;
    }
    ...
    try {
      await ref.read(broadbandRepositoryProvider).createAccount(
            accountNumber: acc,
      ...
      ref.invalidate(broadbandSavedAccountsProvider);
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    final biller = ref.watch(selectedBroadbandBillerProvider);
    return Scaffold(
