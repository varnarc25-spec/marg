...
import 'broadband_autopay_page.dart';
import 'broadband_payment_success_page.dart';

/// Bill details + payment, then `POST /pay`.
class BroadbandBillBreakdownPage extends ConsumerStatefulWidget {
  const BroadbandBillBreakdownPage({super.key});

  @override
  ConsumerState<BroadbandBillBreakdownPage> createState() => _BroadbandBillBreakdownPageState();
}

class _BroadbandBillBreakdownPageState extends ConsumerState<BroadbandBillBreakdownPage> {
  ...
  Widget build(BuildContext context) {
    final bill = ref.watch(fetchedBroadbandBillProvider);
        ...
        children: [
          ...
          ListTile(
            ...
            onTap: () => Navigator.push<void>(
              context,
              MaterialPageRoute<void>(builder: (_) => const BroadbandAutopayPage()),
            ),
          ),
          const SizedBox(height: 16),
          ...
          Consumer(
            builder: (context, ref, _) {
              final accountsAsync = ref.watch(broadbandSavedAccountsProvider);
              return accountsAsync.when(
                data: (accounts) {
                  ...
                  return Align(
                    ...
                    child: TextButton(
                      onPressed: () async {
                        final picked = await showModalBottomSheet<BroadbandSavedAccount>(
                          context: context,
                          showDragHandle: true,
          ...
          FilledButton(
            onPressed: _paying
                ...
                : () async {
                    ...
                    try {
                      await ref.read(broadbandRepositoryProvider).payBill(
                            billerId: biller.id,
                            bill: bill,
                            paymentMode: _paymentMode,
                            accountId: accountId,
                          );
                      ref.invalidate(broadbandHistoryProvider);
                      if (!context.mounted) return;
                      await Navigator.push<void>(
                        context,
                        MaterialPageRoute<void>(
                          builder: (_) => const BroadbandPaymentSuccessPage(),
                        ),
                      );
                    } catch (e) {
                      if (!context.mounted) return;
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text(broadbandApiUserMessage(e))),
                      );
                    } finally {
