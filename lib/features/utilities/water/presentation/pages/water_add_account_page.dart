...
/// `POST /api/utilities/water/accounts`
class WaterAddAccountPage extends ConsumerStatefulWidget {
  const WaterAddAccountPage({super.key});

  @override
  ConsumerState<WaterAddAccountPage> createState() =>
      _WaterAddAccountPageState();
}

class _WaterAddAccountPageState extends ConsumerState<WaterAddAccountPage> {
  ...
  Future<void> _save() async {
    ...
    if (biller == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Select a biller from Water flow first'),
        ),
      );
      return;
    }
    ...
    try {
      await ref.read(waterRepositoryProvider).createAccount(
            accountNumber: acc,
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(waterApiUserMessage(e))),
      );
    } finally {
  ...
  Widget build(BuildContext context) {
    final biller = ref.watch(selectedWaterBillerProvider);
    return Scaffold(
