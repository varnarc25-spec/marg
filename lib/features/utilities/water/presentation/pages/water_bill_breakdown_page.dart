...
import 'water_autopay_page.dart';
import 'water_payment_success_page.dart';

/// Bill details + payment, then `POST /pay`.
class WaterBillBreakdownPage extends ConsumerStatefulWidget {
  const WaterBillBreakdownPage({super.key});

  @override
  ConsumerState<WaterBillBreakdownPage> createState() =>
      _WaterBillBreakdownPageState();
}

class _WaterBillBreakdownPageState
    extends ConsumerState<WaterBillBreakdownPage> {
  ...
  Widget build(BuildContext context) {
    final bill = ref.watch(fetchedWaterBillProvider);
        ...
        children: [
          ...
          ListTile(
            ...
            onTap: () => Navigator.push<void>(
              ...
              MaterialPageRoute<void>(
                builder: (_) => const WaterAutopayPage(),
              ),
            ),
          ),
          const SizedBox(height: 16),
          ...
          Consumer(
            builder: (context, ref, _) {
              final accountsAsync = ref.watch(waterSavedAccountsProvider);
                ...
                data: (accounts) {
                  ...
                  return Align(
                    ...
                    child: TextButton(
                      onPressed: () async {
                        final picked =
                            await showModalBottomSheet<WaterSavedAccount>(
                          context: context,
                          showDragHandle: true,
          ...
          FilledButton(
            onPressed: _paying
                ...
                : () async {
                    ...
                    try {
                      await ref.read(waterRepositoryProvider).payBill(
                            billerId: biller.id,
                            bill: bill,
                      ...
                      await Navigator.push<void>(
                        context,
                        MaterialPageRoute<void>(
                          builder: (_) => const WaterPaymentSuccessPage(),
                        ),
                      );
                    } catch (e) {
                      if (!context.mounted) return;
