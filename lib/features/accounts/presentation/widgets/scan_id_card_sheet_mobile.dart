...
/// Detect image format from magic bytes and return file extension (without dot).
...
/// [margApi] and [idToken] are used on web for server-side OCR; on mobile local ML Kit is used.
Future<ExtractedIdData?> showScanIdCardSheet(
  ...
  return showModalBottomSheet<ExtractedIdData?>(
    ...
    builder: (ctx) => const _ScanIdCardSheet(),
  );
}

class _ScanIdCardSheet extends StatefulWidget {
  const _ScanIdCardSheet();

  @override
  State<_ScanIdCardSheet> createState() => _ScanIdCardSheetState();
}

class _ScanIdCardSheetState extends State<_ScanIdCardSheet> {
  ...
  Future<void> _pickAndRecognize(ImageSource source) async {
    ...
    try {
      ...
      final tempFile = File(p.join(tempDir.path, 'id_scan_${DateTime.now().millisecondsSinceEpoch}.$ext'));
      await tempFile.writeAsBytes(bytes);

      ExtractedIdData? result;
      try {
        result = await _ocrService.extractFromFile(tempFile);
      } finally {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'Take a picture or upload a photo of your PAN or Aadhaar card to auto-fill details.',
